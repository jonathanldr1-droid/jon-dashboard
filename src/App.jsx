...
Give advice specific to this hardware. Reference M4250 AV features and UXG-Express-7 capabilities and limitations. Be practical, use numbered steps, recommend specific models where helpful. Never use em dashes.`, messages: history, }), }); const data = await res.json(); setItMessages(m => [...m, { role: "assistant", content: data.content?.[0]?.text || "Something went wrong." }]); } catch (e) { setItMessages(m => [...m, { role: "assistant", content: `Error: ${e.message}` }]);
}
setItLoading(false);
};
